...
const attachSessionCacheHeaders = (
  res: Response,
...
const serializeSession = (session: any) => {
  const toIso = (val: unknown): string | null => {
    ...
    // Si ya es un Date object (pg devuelve timestamptz como Date)
    if (val instanceof Date) {
      return val.toISOString();
    }
    // Si es string, parsear y convertir
  ...
  return {
    ...
    media_connect_ts: toIso(session.media_connect_ts),
    media_start_ts: toIso(session.media_start_ts),
    media_end_ts: toIso(session.media_end_ts),
    recommended_start_offset_ms:
      typeof session.recommended_start_offset_ms === "number"
        ? session.recommended_start_offset_ms
        : null,
  };
};
...
router.get("/range", async (req: Request, res: Response) => {
  try {
    ...
    if (!fromDate) {
      return res
        .status(400)
        .json({ error: "from query parameter is required (ISO string)" });
    }
    ...
    const sessions = await db.listSessionsByTimeRange(
      fromDate,
      toDate,
      safeLimit
    );
...
router.get("/:sessionId/meta", async (req: Request, res: Response) => {
  try {
    ...
    if (session) {
      const sessionPath =
        typeof session.path === "string" ? session.path : undefined;
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!Number.isFinite(segmentId) || segmentId < 0) {
        return res
          .status(400)
          .json({ error: "segmentId must be a non-negative integer" });
      }
...
router.post("/open", async (req: Request, res: Response) => {
  try {
    ...
    if (typeof startTs !== "string" || !parseIsoDate(startTs)) {
      return res
        .status(400)
        .json({ error: "startTs must be a valid ISO string" });
    }

    const streamPath =
      typeof path === "string" && path.trim().length > 0 ? path : devId;
...
router.post("/close", async (req: Request, res: Response) => {
  try {
    ...
    if (typeof endTs !== "string" || !parseIsoDate(endTs)) {
      return res
        .status(400)
        .json({ error: "endTs must be a valid ISO string" });
    }
...
router.get("/:sessionId/clip", async (req: Request, res: Response) => {
  try {
    ...
    const format =
      typeof req.query.format === "string" && req.query.format.trim().length > 0
        ? req.query.format
        : "mp4";
    ...
    // Usar media_start_ts como ancla si existe (timestamp real del primer segmento)
    // De lo contrario usar start_ts + offset como fallback
    let anchorDate: Date;
    let anchorSource: string;

    if (session.media_start_ts) {
      const mediaStart = parseIsoDate(session.media_start_ts);
      if (mediaStart) {
        // Agregar pequeño offset si está configurado (50-100ms para compensar variaciones)
        const offsetMs = session.recommended_start_offset_ms ?? 0;
        anchorDate = new Date(mediaStart.getTime() + offsetMs);
        anchorSource = "media_start_ts";
      } else {
        anchorDate = new Date(
          startDate.getTime() + CONFIG.PLAYBACK_START_OFFSET_MS
        );
        anchorSource = "fallback_offset";
      }
    } else {
      // Fallback: usar offset legacy
      anchorDate = new Date(
        startDate.getTime() + CONFIG.PLAYBACK_START_OFFSET_MS
      );
      anchorSource = "fallback_offset";
    ...
    // Usar media_end_ts si existe, sino end_ts
    let endAnchorDate: Date;
    if (session.media_end_ts) {
      const mediaEnd = parseIsoDate(session.media_end_ts);
      endAnchorDate = mediaEnd || endDate;
    } else {
      endAnchorDate = endDate;
    }

    // Calcular duration desde el ancla de inicio hasta el ancla de fin
    const durationMs = Math.max(
      0,
      endAnchorDate.getTime() - anchorDate.getTime()
    );
    ...
    console.log(
      JSON.stringify({
        ...
        recommended_offset_ms: session.recommended_start_offset_ms ?? 0,
      })
    );
    ...
    playbackBase.searchParams.set("start", anchorDate.toISOString());
