    ...
    try {
      ...
      for (const det of meta.detections) {
        if (!det.trackId || typeof det.trackId !== 'string') {
          return res.status(400).json({ error: 'Invalid trackId in detection' });
        }
      ...
      const tempFramePath = path.join(FRAMES_DIR, `temp_${meta.sessionId}_${meta.seqNo}.jpg`);
      await fs.writeFile(tempFramePath, frameBuffer);

      // Obtener detecciones existentes para esta sesión (una sola query)
      const existingDetections = await db.getDetectionsBySession(meta.sessionId);
      const existingMap = new Map(existingDetections.map((d) => [d.track_id, d]));
      ...
      for (const det of meta.detections) {
        ...
        const framePath = path.join(FRAMES_DIR, `${meta.sessionId}_${det.trackId}.jpg`);
