#!/bin/sh
# Hook ejecutado cuando MediaMTX inicia un segmento de grabación
...
if [ "$PATH_VALUE" = "unknown" ] || [ -z "$SEGMENT_PATH" ]; then
  echo "[segment_start.sh] WARNING: Missing variables" >&2
...
if [ -n "$HOOK_TOKEN" ]; then
  curl -X POST "$STORE_URL/hooks/mediamtx/record/segment/start" \
