...
export async function detectEncoder(): Promise<EncoderConfig> {
  ...
  logger.warn(`No hardware encoder found, using software fallback`, { encoder: fallback.element });
  return fallback;
}
  ...
  return new Promise((resolve) => {
    ...
    const testArgs =
      encoder === "vaapih264enc"
        ? [...base, "video/x-raw,format=NV12,width=64,height=64", "!", "vaapih264enc", "bitrate=1000", "rate-control=cbr", "!", "fakesink"]
        : [...base, "video/x-raw,width=64,height=64", "!", encoder, "!", "fakesink"];
