...
class FrameDecoder:
    ...
    def _register_default_decoders(self):
        ...
        # JPEG
        self.register(codec=pb.CODEC_JPEG, pixel_format=None, decoder=self._decode_jpeg)

        # NV12
        self.register(
            codec=pb.CODEC_NONE, pixel_format=pb.PF_NV12, decoder=self._decode_nv12
        )

        # I420
        self.register(
            codec=pb.CODEC_NONE, pixel_format=pb.PF_I420, decoder=self._decode_i420
        )

    def register(self, codec: int, pixel_format: Optional[int], decoder: DecoderFunc):
        """
        ...
        logger.debug(
            f"Decodificador registrado: codec={codec}, pixel_format={pixel_format}"
        )

    def decode(
        self, data: bytes, codec: int, pixel_format: int, width: int, height: int
    ) -> Optional[np.ndarray]:
        """
        ...
        logger.error(
            f"No hay decodificador para codec={codec}, pixel_format={pixel_format}"
        )
        return None
    ...
    def _decode_nv12(data: bytes, width: int, height: int) -> Optional[np.ndarray]:
        ...
        try:
            ...
            if len(data) < y_size + uv_size:
                logger.error(
                    f"Frame NV12 incompleto: esperado {y_size + uv_size}, recibido {len(data)}"
                )
                return None
