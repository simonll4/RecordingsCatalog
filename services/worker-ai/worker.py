...
class ModelManager:
    ...
    def infer(self, frame_rgb: np.ndarray) -> ai_pb2.Result:
...
class ConnectionHandler:
    ...
    async def handle_frame(self, frame: ai_pb2.Frame):
        ...
        logger.debug(f"Processing frame seq={frame.seq}, size={len(frame.data)}")
        ...
        result = self.model_manager.infer(frame_rgb)
